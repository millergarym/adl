...
use adl_core::adlgen::adlc::workspace::TypescriptGenOptions;
use adl_core::adlgen::sys::adlast2::{
    Annotations, Decl1, DeclType, Module1, NewType, PrimitiveType, ScopedName, Struct1, TypeDef,
    TypeExpr, TypeRef, Union1,
};
use adl_core::parser::docstring_scoped_name;
use adl_core::processing::resolver::Resolver;
use adl_core::utils::ast::get_type_params;
                  ...
                  return {
                    $(for f in &m.fields => $(ref t => {
                        if let Some(_) = f.default.0 {
                            ...
                            let dvg = &mut crate::defaultval::TsDefaultValue{
                                ctx: &crate::defaultval::ResolverModule {
                                    module: self.module,
        ...
        quote_in! { *t =>
            ...
            const $(cap_opt(name, self.opts))_AST : $(&self.adlr).ScopedDecl =
              {"moduleName":$("\"")$(mname.clone())$("\""),"decl":$(ref tok => {
                let mut sdg = crate::astgen::TsScopedDeclGenVisitor{module_name: &mname.clone(), t: tok};
                sdg.visit_decl(decl);
              })};$['\n']
