...
use adl_core::adlgen::adlc::bundle::{AdlBundle, NpmOptions};
use adl_core::adlgen::adlc::workspace::{
    AdlBundleRefType, AdlWorkspace, ModuleSrc, NpmPackage, Payload1, TsRuntimeOpt, TsStyle,
    TsWriteRuntime, TypescriptGenOptions,
};
use adl_core::adlgen::sys::adlast2::Module1;
use adl_core::adlgen::sys::adlast2::{self as adlast};
use adl_core::adlstdlib::get_file_names;
// use crate::cli::tsgen::utils::{get_npm_pkg, npm_pkg_import, IndexEntry};
use adl_core::processing::loader::AdlLoader;
use adl_core::processing::resolver::Resolver;
use adl_core::processing::writer::TreeWriter;

use rust_embed::RustEmbed;

use crate::utils::{IndexEntry, get_npm_pkg, npm_pkg_import};
use crate::{generate, utils};

#[derive(RustEmbed)]
#[folder = "src/ts-runtime/"]
struct Asset;
...
pub fn tsgen(
    ...
    let (manifest, outputdir) = match outputs {
        adl_core::adlgen::adlc::workspace::OutputOpts::Gen(gen) => (
